    ...
    ServerConnection? serverConnection)
{
    ...
    private static List<LogicOfferBundle> BuildShopOffers()
    {
        var result = new List<LogicOfferBundle>();
        try
        {
            var endTime = (int)(DateTimeOffset.UtcNow.AddDays(1).ToUnixTimeSeconds());
            foreach (var offer in ShopConfig.Data.Offers)
            {
                var gemOffers = offer.Items.Select(item =>
                {
                    var shopType = item.Type switch
                    {
                    ...
                    var go = new LogicGemOffer(shopType, item.Amount);
                    if (item.SkinId > 0) go.SetSkin(item.SkinId);
                    return go;
                }).ToList();
        ...
        {
            Console.WriteLine($"[Shop] BuildShopOffers error: {e.Message}");
        }
        return result;
    }
}
